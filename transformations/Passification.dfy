...
module Passification
{
  ...
  */
  ...
  datatype OutputIncRes = OutputIncRes(map<var_name, nat>, Incarnation<var_name>, BasicBlock)

  function method OutputIncarnation(globalIncMap: Incarnation<nat>, input: Incarnation<var_name>, b: BasicBlock) : OutputIncRes
  requires input.Keys <= globalIncMap.Keys
  ensures
    var OutputIncRes(globalIncMap', outputInc, b') := OutputIncarnation(globalIncMap, input, b);
    && outputInc.Keys <= globalIncMap'.Keys
    && globalIncMap.Keys <= globalIncMap'.Keys
  decreases b
  {
    match b
    case Skip => OutputIncRes(globalIncMap, input, b)
    case Assert(e) => 
      var e' := e.MultiSubstExpr(input); 
      OutputIncRes(globalIncMap, input, Assert(e'))
    case Assume(e) => 
      var e' := e.MultiSubstExpr(input); 
      OutputIncRes(globalIncMap, input, Assume(e'))
    case Assign(x, t, e) => 
      var e' := e.MultiSubstExpr(input); 
      var counter := if x in globalIncMap.Keys then globalIncMap[x]+1 else 1;
      var x' := VersionedName(x, counter);

      OutputIncRes(globalIncMap[x := counter], input[x := x'], Assign(x', t, e'))
    ...
    case Havoc(varDecls) => 
      ...
      OutputIncRes(globalIncMap', output, Havoc(varDecls'))
    case SeqSimple(sc1, sc2) => 
      var OutputIncRes(globalIncMap1, inc1, sc1') := OutputIncarnation(globalIncMap, input, sc1);
      var OutputIncRes(globalIncMap2, inc2, sc2') := OutputIncarnation(globalIncMap1, inc1, sc2);
      OutputIncRes(globalIncMap2, inc2, SeqSimple(sc1',sc2'))
  }
    ...
    decreases |topo|-idTopo
  {
    if idTopo == |topo| then 
      ...
      var OutputIncRes(globalIncMap'', outputIncarnation, b') := OutputIncarnation(incRes.globalIncMap, incRes.localIncMap, cfg.blocks[curId]);
